...
mod tests {
    ...
    #[tokio::test]
    async fn test_get_lat_lon_error() {
        let _m = mock("GET", "/search?postalcode=99999&format=json")
            .with_status(404)
            .create();

        let result = get_lat_lon(LocationInput::PostalCode("99999".to_string()), Some(&mockito::server_url())).await;
